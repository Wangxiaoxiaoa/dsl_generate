...
class DspyTransform(dspy.Signature):
    prompt = dspy.InputField(desc="完整的提示词，包括语法规则、辅助说明、错误的语句和特殊token")
...
class Transform(dspy.Module):
    ...
    def forward(self, grammar, antlr_help, sentence, special_token1, special_token2,error_msg):
...
def transform(model_name, api_key, url, grammar,prompt, antlr_help, sentence, special_token:list,error_msg=None):
